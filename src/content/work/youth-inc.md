...
roles:
  - title: Senior Software Engineer
    ...
    highlights:
      - Owned the order-fulfillment integration layer end to end — 100% of company order volume, roughly 6,100 orders a year — routing across ShipStation (REST/webhooks), LINX (SOAP), and FulfillEngine with idempotent submission and reconciliation, plus the shipment-tracking pipeline that notifies customers on ship.
      ...
      - Closed a critical disaster-recovery gap with automated production database backups, taking recovery point objective from unbounded to a fixed window, plus prod-to-local restore tooling that became the team's standard debugging workflow.
      - Bootstrapped and owned the Phoenix LiveView commerce platform used daily by operations — inventory ingestion with dry-run diffs, SKU launch automation, per-decorator fulfillment dashboards, demand forecasting with at-risk SKU detection, and per-SKU margin reporting — replacing SSH-into-production console workflows with self-serve tooling.
